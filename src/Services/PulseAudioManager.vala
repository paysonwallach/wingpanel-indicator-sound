...
public class Sound.Services.PulseAudioManager : GLib.Object {
    ...
    public signal void new_device (Sound.Services.Device dev);

    public PulseAudio.Context context { get; private set; }
    private PulseAudio.GLibMainLoop loop;
    private bool is_ready = false;
    private uint reconnect_timer_id = 0U;
    private Gee.HashMap<string, Sound.Services.Device> input_devices;
    private Gee.HashMap<string, Sound.Services.Device> output_devices;
    public Sound.Services.Device default_output { get; private set; }
    public Sound.Services.Device default_input { get; private set; }
    ...
    construct {
        ...
        input_devices = new Gee.HashMap<string, Sound.Services.Device> ();
        output_devices = new Gee.HashMap<string, Sound.Services.Device> ();
    ...
    public async void set_default_device (Sound.Services.Device device) {
        debug ("\n");
    ...
    private async void wait_for_update (Sound.Services.Device device, string prop_name) {
    ...
    public void change_device_mute (Sound.Services.Device device, bool mute = true) {
        if (device.input) {
    ...
    public void change_device_volume (Sound.Services.Device device, double volume) {
    ...
    public void change_device_balance (Sound.Services.Device device, float balance) {
        var cvol = device.cvolume;
    ...
    private void card_info_callback (PulseAudio.Context c, PulseAudio.CardInfo? card, int eol) {
        ...
        foreach (var port in relevant_ports) {
            ...
            Gee.HashMap<string, Sound.Services.Device> devices = is_input? input_devices : output_devices;
            Sound.Services.Device device = null;
            var id = get_device_id (card, port);
            bool is_new = !devices.has_key (id);
            if (is_new) {
                debug ("\t\t\tnew device: %s", id);
                device = new Sound.Services.Device (id, card.index, port.name);
            } else {
    ...
    private void cleanup_devices (Gee.HashMap<string, Sound.Services.Device> devices, PulseAudio.CardInfo card, PulseAudio.CardPortInfo*[] relevant_ports) {
    ...
    private Gee.ArrayList<string> get_relevant_card_port_profiles (PulseAudio.CardPortInfo* port) {
        ...
        // just store names in Sound.Services.Device
    ...
    private void remove_devices_by_card (Gee.HashMap<string, Sound.Services.Device> devices, uint32 card_index) {
        var iter = devices.map_iterator ();
        while (iter.next ()) {
            var device = iter.get_value ();
